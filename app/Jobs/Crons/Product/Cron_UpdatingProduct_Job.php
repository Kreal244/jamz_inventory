...
class Cron_UpdatingProduct_Job implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $update_string = [];
        // todo: remove special key from the input type number and return notice if "SKU is conflicted"
        try {
            foreach ($action_list as $key => $value) {
                ...
                $check_available = DB::select("SELECT EXISTS(SELECT * FROM products WHERE product_id != " . $pdo->quote($value['product_id']) . " and sku_10digits = '" . $value["sku_10digits"] . "') as is_available");
                if ($check_available[0]->is_available) {
                    throw new Exception("SKU already exist");
                }
                $update_string[] = "(" .
                    $pdo->quote($value['product_id']) . "," .
                    ...
                    (!empty($value['extra8']) ? $value['extra8'] : "NULL") . "," .
                    ...
                    (!empty($value['per_case']) ? Utils::formatNumber($value['per_case']) : "NULL") . "," .
                    ...
                    (!empty($value['in_container_40_pallets']) ? $value['in_container_40_pallets'] : "NULL") . "," .
                ...
                $categories[] = "(" . $pdo->quote(md5($value['category'])) . "," . $pdo->quote(strtolower($value['category'])) . ")";
                $product_cate_xref[] = "(" . $pdo->quote($value['product_id']) . "," . $pdo->quote(md5($value['category'])) . ")";
            }
            if (!empty($update_string)) {
                $result = ProductModel::updateProductSheet($update_string, $manufacturers, $product_cate_xref, $categories); //get list items' id after updating
                if ($result) {
            ...
            // throw new Exception("There are some wrongs when udpating product.");
